...
class Form
{
    ...
    public function __construct( $resource = 'auto', $action = 'update', $itemId = null )
    {
        ...
        do_action('sombrillawp_after_form_element_init', $this);
    }
    ...
    protected function autoConfig()
    {
        if ($this->resource === 'auto') {
            ...
            //     $resource = 'option';
            // }
            $resource = 'option'; //Forzado
             $item_id  = null; //Forzado
            $this->itemId = $item_id;
            $this->resource = strtolower($resource);
        }
    ...
    public function setScreen($screen) {
        ...
        echo $this->hidden('sombrillawp_screen')->setAttribute('value', $screen)->setAttribute('name', 'sombrillawp_screen');
         echo $this->hidden('sombrillawp_prefix')->setAttribute('value', $this->prefix)->setAttribute('name', 'sombrillawp_prefix');

        if ($screen == 'options') {
            if ($this->getPrefix() == 'swp') {
                $prefix = (isset( $_GET['page'] )) ? $_GET['page']: 'swp';
            ...
            $this->data = get_option($this->prefix, []);
            $this->fill = true;
        }elseif ($screen == 'post_meta') {   
            GLOBAL $post;
            $this->data = get_post_meta($post->ID, $this->prefix, true);
            $this->fill = true;
        }
        ...
        foreach ($fields as $field) {
            ...
            if($field instanceof Field) {
                $clone_field = clone $field;
                 $data = $clone_field->configureToForm($this);
